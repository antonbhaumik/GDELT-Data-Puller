...
# Now, we smooth with a 14-day moving average, then get values val>3*std+mean as outliers. Next, we group the articles
# by dates close to each other, then take only the articles of the date in the group with the most published articles.
df['Moving Average'] = df['Value'].rolling(window=14, center=True).mean()
df['Differences'] = df['Value'] - df['Moving Average']  # We care only about peaks, not troughs.
...
plt.grid(True)
plt.savefig('analysis/volume.png', dpi=300, bbox_inches='tight')
plt.legend()
plt.show()
...
df.to_csv('analysis/volume_outliers.csv', index=False)
